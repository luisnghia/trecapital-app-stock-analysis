...
class SQLiteChecklistRepository:
    ...
    def _snapshot_payload(self,c,rid):
        ...
        return {'snapshot_schema':'phase1b-review-v5-evidence-peer-ai-management','generated_at':datetime.now(timezone.utc).isoformat(timespec='seconds'),'company':co,'review':rv,'metrics':self.review_metrics(rid,conn=c),'quality_tally':self.quality_tally(rid,conn=c),'assessments':self.latest_assessments_for_review(rid,conn=c),'screening':self.latest_screening_for_review(rid,conn=c),'opportunity_inventory':dict(inv) if inv else None,'research_evidence':snapshot_evidence_for_review(self,rid,conn=c),'peer_comparison':snapshot_peer_payload_for_review(self,rid,conn=c),'ai_research':snapshot_ai_for_review(self,rid,conn=c),'management_intelligence':snapshot_management_for_review(self,rid,conn=c)}
