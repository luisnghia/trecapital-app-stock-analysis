...
def test_maintenance_capex_keeps_trecapital_value_separate_from_depreciation_proxy():
    df = pd.DataFrame([
        {
            "period": "2025", "period_type": "Y", "year": 2025,
    ...
    assert row["Maintenance Capex Trecapital"] == pytest.approx(90)
    assert row["D&A rough proxy"] == pytest.approx(80)
    assert row["Maintenance Capex Trecapital"] != row["D&A rough proxy"]
