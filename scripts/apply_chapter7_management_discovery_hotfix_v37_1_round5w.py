...
def replace_once(text: str, old: str, new: str, label: str) -> str:
    if new in text:
        return text
    if old not in text:
        raise RuntimeError(f"V37.1 Round 5W patch marker not found: {label}")
...
def patch_discovery() -> None:
    ...
    """Extract a Chairman candidate from a tightly local board-signature block.

    Vietnamese official resolutions commonly end with a signature block such as::

        T/M. HỘI ĐỒNG QUẢN TRỊ
        CHỦ TỊCH
        Nguyễn Văn An

    `CHỦ TỊCH` alone is intentionally *not* a global Chairman alias because it can refer to other
    bodies. It is accepted here only when a Board-of-Directors context line is within the preceding
    two lines and a plausible bare person name is on the same or next two lines. `PHÓ CHỦ TỊCH` is
    explicitly excluded. The result remains a research candidate requiring analyst verification.
    """
    ...
    for idx, line in enumerate(lines):
        ...
        if suffix:
            candidate_lines.append((suffix, line))
        for j in range(idx + 1, min(len(lines), idx + 3)):
            next_line = _clean_text(lines[j])
            # Stop if a competing management role begins before a name is found.
            next_role, _, _ = _role_span_in_line(next_line)
            if next_role[1] and next_role[1] != "Chairman":
                break
            candidate_lines.append((next_line, f"{line} {next_line}"))

        for candidate_text, local_context in candidate_lines:
            manager = ""
            ...
            out.append((manager, ("chủ tịch", "Chairman", 100), context or _clean_text(local_context)[:900]))
            break
...
''' + marker
    ...
    text = replace_once(text, old_signal, new_signal, "global management signal board signature")
    ...
    text = replace_once(text, old_requested, new_requested, "requested role signature signal")

    old_rows_end = '''        for manager, (role_raw, role_norm, priority), context in _role_then_name_candidates(raw_text):\n            rows.append({\n                "Select": False,\n                "Manager": manager,\n                "Role Raw": role_raw,\n                "Role Normalized": role_norm,\n                "As-of Date": as_of,\n                "Source Title": source_title[:240],\n                "Source URL / File": source_url,\n                "Source Grade": "A — Company/Official disclosure",\n                "Evidence Text / Reference": context[:900],\n                "Status": "Discovered candidate — analyst verify",\n                "_priority": priority,\n            })\n    if not rows:\n'''
    new_rows_end = '''        for manager, (role_raw, role_norm, priority), context in _role_then_name_candidates(raw_text):\n            rows.append({\n                "Select": False,\n                "Manager": manager,\n                "Role Raw": role_raw,\n                "Role Normalized": role_norm,\n                "As-of Date": as_of,\n                "Source Title": source_title[:240],\n                "Source URL / File": source_url,\n                "Source Grade": "A — Company/Official disclosure",\n                "Evidence Text / Reference": context[:900],\n                "Status": "Discovered candidate — analyst verify",\n                "_priority": priority,\n            })\n        for manager, (role_raw, role_norm, priority), context in _board_signature_candidates(raw_text):\n            rows.append({\n                "Select": False,\n                "Manager": manager,\n                "Role Raw": role_raw,\n                "Role Normalized": role_norm,\n                "As-of Date": as_of,\n                "Source Title": source_title[:240],\n                "Source URL / File": source_url,\n                "Source Grade": "A — Company/Official disclosure",\n                "Evidence Text / Reference": context[:900],\n                "Status": "Discovered candidate — analyst verify",\n                "_priority": priority,\n            })\n    if not rows:\n'''
    text = replace_once(text, old_rows_end, new_rows_end, "extract board signature rows")
...
def patch_tests() -> None:
    ...
    sentinel = "def test_board_signature_block_extracts_chairman_v37_1_round5w():"
    if sentinel in text:
        return
    text += r'''


def test_board_signature_block_extracts_chairman_v37_1_round5w():
...
def test_board_signature_does_not_promote_vice_chairman_to_chairman_v37_1_round5w():
    import modules.deep_company_analysis.chapter7_management_discovery as md
...
def main() -> None:
    patch_discovery()
    patch_tests()
    print("Chapter 7 V37.1 Round 5W board-signature Chairman parser applied")
