...
'''
    ...
    workload_helpers = "\ndef _qlkh_workload_counts(day_start, day_end, include_backlog=False):\n    # Aggregate only: never expose another QLKH's customers, notes or scores.\n    lower = \"\" if include_backlog else \" AND COALESCE(t.assigned_at,t.created_at,t.start_time)>=?\"\n    params = [day_end]\n    if not include_backlog:\n        params.append(day_start)\n    return qdf(\"\"\"SELECT s.id support_id,s.full_name support_name,s.username,\n                         s.active,t.task_type,\n                         CASE WHEN t.status='PENDING_ACCEPTANCE' THEN 'Chờ tiếp nhận'\n                              WHEN t.status IN ('OPEN','REWORK') THEN 'Đang xử lý' END phase,\n                         COUNT(t.id) task_count\n                  FROM users s\n                  LEFT JOIN tasks t ON t.support_user_id=s.id\n                     AND t.status IN ('PENDING_ACCEPTANCE','OPEN','REWORK')\n                     AND COALESCE(t.assigned_at,t.created_at,t.start_time)<?\"\"\" + lower + \"\"\"\n                  WHERE s.role='Cán bộ hỗ trợ'\n                  GROUP BY s.id,s.full_name,s.username,s.active,t.task_type,phase\n                  HAVING s.active=1 OR COUNT(t.id)>0\"\"\", tuple(params))\n\n\ndef _render_qlkh_workload(u):\n    if u.get(\"role\") != \"Cán bộ QLKH\":\n        return\n    from datetime import datetime, timedelta\n    from zoneinfo import ZoneInfo\n    import plotly.graph_objects as go\n    current = datetime.now(ZoneInfo(\"Asia/Ho_Chi_Minh\"))\n    day = current.strftime(\"%Y-%m-%d\")\n    following = (current + timedelta(days=1)).strftime(\"%Y-%m-%d\")\n    with st.expander(\"📊 Tải công việc CBHT hôm nay\", expanded=True):\n        st.caption(\"CBHT = Cán bộ hỗ trợ. Số hồ sơ toàn phòng còn chờ tiếp nhận hoặc đang xử lý, gồm cả hồ sơ yêu cầu làm lại; không gồm hồ sơ đã báo hoàn thành, đã trả lại hoặc đã hủy.\")\n        backlog = st.checkbox(\"Tính cả hồ sơ tồn từ các ngày trước\", value=False, key=\"qlkh_workload_backlog\")\n        refresh = st.button(\"↻ Cập nhật số lượng\", key=\"qlkh_workload_refresh\")\n        rows = _qlkh_workload_counts(day + \" 00:00:00\", following + \" 00:00:00\", backlog)\n        scope = \"tất cả hồ sơ tồn đến hôm nay\" if backlog else \"hồ sơ giao trong ngày \" + current.strftime(\"%d/%m/%Y\")\n        st.caption(\"Phạm vi: \" + scope + \" · Cập nhật \" + current.strftime(\"%H:%M:%S\") + \" (giờ Việt Nam).\")\n        if rows.empty:\n            st.info(\"Chưa có cán bộ hỗ trợ để thống kê.\")\n            return\n        types = sorted(rows.loc[rows[\"task_count\"].gt(0), \"task_type\"].dropna().astype(str).unique().tolist())\n        choices = [\"Tất cả loại công việc\"] + types\n        if st.session_state.get(\"qlkh_workload_type\") not in choices:\n            st.session_state[\"qlkh_workload_type\"] = choices[0]\n        selected = st.selectbox(\"Loại công việc cần phân giao\", choices, key=\"qlkh_workload_type\")\n        staff = rows[[\"support_id\",\"support_name\",\"username\",\"active\"]].drop_duplicates(\"support_id\").copy()\n        staff[\"label\"] = staff[\"support_name\"].astype(str) + \" (\" + staff[\"username\"].astype(str) + \")\"\n        staff.loc[staff[\"active\"].eq(0), \"label\"] += \" · Đã khóa\"\n        counts = rows[rows[\"task_count\"].gt(0)].copy()\n        if selected != choices[0]:\n            counts = counts[counts[\"task_type\"].eq(selected)]\n        totals = counts.groupby(\"support_id\")[\"task_count\"].sum()\n        staff[\"total\"] = staff[\"support_id\"].map(totals).fillna(0).astype(int)\n        staff = staff.sort_values([\"total\",\"label\"], ascending=[False,True])\n        fig = go.Figure()\n        for phase, color in [(\"Đang xử lý\",\"#00736C\"),(\"Chờ tiếp nhận\",\"#F4B41A\")]:\n            subset = counts[counts[\"phase\"].eq(phase)]\n            by_staff = subset.groupby(\"support_id\")[\"task_count\"].sum()\n            details = {sid: \"; \".join(str(r.task_type) + \": \" + str(int(r.task_count)) for r in group.itertuples())\n                       for sid, group in subset.groupby(\"support_id\")}\n            values = staff[\"support_id\"].map(by_staff).fillna(0).astype(int)\n            fig.add_trace(go.Bar(name=phase, y=staff[\"label\"], x=values, orientation=\"h\",\n                marker_color=color, text=[str(v) if v else \"\" for v in values], textposition=\"inside\",\n                insidetextfont={\"color\":\"white\" if phase==\"Đang xử lý\" else \"#17231F\"},\n                customdata=staff[\"support_id\"].map(details).fillna(\"Không có hồ sơ\"),\n                hovertemplate=\"%{y}<br>\" + phase + \": %{x} hồ sơ<br>%{customdata}<extra></extra>\"))\n        fig.update_layout(barmode=\"stack\", height=max(260, 52*len(staff)+110),\n            margin=dict(l=0,r=15,t=10,b=10), legend=dict(orientation=\"h\",y=1.12,x=0),\n            xaxis=dict(title=\"Số hồ sơ\",dtick=1,rangemode=\"tozero\"),\n            yaxis=dict(title=None,autorange=\"reversed\"), bargap=0.3)\n        st.plotly_chart(fig,use_container_width=True,key=\"qlkh_workload_chart\",config={\"displayModeBar\":False})\n        st.caption(\"Đang xử lý: \" + str(int(counts.loc[counts[\"phase\"].eq(\"Đang xử lý\"),\"task_count\"].sum())) +\n                   \" · Chờ tiếp nhận: \" + str(int(counts.loc[counts[\"phase\"].eq(\"Chờ tiếp nhận\"),\"task_count\"].sum())) +\n                   \" · CBHT chưa có hồ sơ trong phạm vi chọn: \" + str(int(staff[\"total\"].eq(0).sum())))\n\n\n"
    source = _replace_once(source, "def qlkh_page(u):\n", workload_helpers + "def qlkh_page(u):\n", "QLKH workload helpers")
    title = '    page_title("Tác nghiệp – Cán bộ QLKH","QLKH giao hồ sơ cho Cán bộ hỗ trợ, theo dõi tiến độ và đánh giá sau khi công việc hoàn thành.")\n'
    source = _replace_once(source, title, title + '    _render_qlkh_workload(u)\n', "QLKH workload chart")
    compile(source, "<qlkh-workload-chart>", "exec")
    return source
