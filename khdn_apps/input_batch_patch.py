...
def patch_source(source: str) -> str:
    # 1) QLKH create/give-file page: customer search remains live outside the form.
    # Everything after customer selection is browser-local until explicit submit.
    source = _wrap_block(
        source,
    ...
    # 2) CBHT self-create page: same browser-local payload pattern.
    source = _wrap_block(
        source,
    ...
    # 3) Reason-category manager: the old implementation rendered BOTH tabs, BOTH
    # tables and BOTH edit panels on every run, even while the user only wanted to
    # type one new name. Replace it with a single selected catalog + single mode.
    # Default mode is Add new and performs zero catalog SELECT/dataframe rendering.
    ...
    kind_label=st.radio(
        ...
        ["↩️ Trả lại","🗑️ Hủy hồ sơ"],
        horizontal=True,
        key="reason_catalog_kind_fast",
    )
    ...
    mode=st.radio(
        "Thao tác",
        ["➕ Thêm mới","📋 Danh sách / chỉnh sửa"],
        horizontal=True,
        key=f"reason_catalog_mode_{kind}",
    )
    st.markdown(f"#### {title}")
...
'''
    ...
    # 4) Task-type manager: Add-new mode is now isolated from the dataframe and
    # edit widgets. No task-type SELECT/dataframe is created while typing a name.
    task_type_block = '''    if admin_view == "types":
        st.subheader("Loại công việc")
        type_mode=st.radio(
            "Thao tác",
            ["➕ Thêm mới","📋 Danh sách / trạng thái"],
            horizontal=True,
            key="task_type_catalog_mode_fast",
        )
...
'''
    source = _replace_span(
        ...
        '    if admin_view == "audit":\n',
        task_type_block,
        'lazy task-type manager',
    )

    # Guardrails: never reintroduce the invalid callback crash and ensure all fast
    # sections were installed exactly once.
    ...
    for marker in required_once:
        if source.count(marker) != 1:
            raise RuntimeError(f"Input fast marker not installed exactly once: {marker}")
